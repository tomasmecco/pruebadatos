...
{
    public partial class FrmViste_ProveedorIngreso : Form
    {
        public FrmViste_ProveedorIngreso()
        {
            InitializeComponent();
        }
        private void Listar()
        ...
        private void Buscar()
        {
            try
            {
                ...
                this.Formato();
                LblTotal.Text = "Total usuarios: " + Convert.ToString(DgvListado.Rows.Count);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message + ex.StackTrace);
        ...
        private void FrmViste_ProveedorIngreso_Load(object sender, EventArgs e)
        {
            this.Listar();
        }
